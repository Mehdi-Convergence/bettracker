...
import {
  getPortfolioStats, getPortfolioBets, getCampaigns, createBet, scanValueBets,
} from "../services/api";
import { LEAGUE_INFO } from "../types";
import type { PortfolioStats, Bet, Campaign, MatchCard } from "../types";
...
export default function Portfolio() {
  ...
  const [scanResults, setScanResults] = useState<MatchCard[]>([]);
  ...
  async function handleScan() {
    setScanLoading(true);
    try {
      const res = await scanValueBets({ demo: "true" });
  ...
  function filteredScanResults(): MatchCard[] {
  ...
  function selectScanMatch(match: MatchCard, outcome: string) {
    const detail = match.outcomes[outcome];
    if (!detail) return;
    ...
    if (isCombo) {
      ...
      setAddForm({
        ...
        match_date: match.date.split("T")[0],
        outcome_bet: outcome,
        odds_at_bet: String(detail.best_odds),
        stake: addForm.stake,
      });
      setFormMode("manual");
    }
                          ...
                          {(["H", "D", "A"] as const).map((o) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <div className="text-amber-600">{detail.best_odds.toFixed(2)}</div>
                                {detail.is_value && (
                                  <div className="text-[9px] text-emerald-600">+{(detail.edge * 100).toFixed(1)}%</div>
                                )}
                              </button>
                            );
                          })}
