"""Train NBA ML model (production) and save to models/nba/.

Steps:
  1. Load all NBA games from DB
  2. Build features with NBAFeatureBuilder (chronological, no look-ahead)
  3. Walk-forward evaluation (2018-19 to 2022-23 train / 2023-24 to 2024-25 test)
  4. Train production model on ALL data
  5. Export team stats snapshot to models/nba/team_stats.json
  6. Save model to models/nba/model.joblib + metadata.json
"""
...
from src.backtest.nba_engine import TRAIN_SEASONS, TEST_SEASONS
...
def load_games() -> pd.DataFrame:
    db = SessionLocal()
    try:
        rows = db.query(NBAGame).order_by(NBAGame.game_date).all()
        data = []
        for g in rows:
            data.append({
                "id": g.id,
...
def main():
    print("Loading NBA games from DB...")
    raw_df = load_games()
    ...
    print("Building NBA features (incremental, no look-ahead)...")
    ...
    if features_df.empty:
        print("ERROR: no features built. Run nba_collector first.")
        sys.exit(1)
    ...
    X_all = features_df[NBA_FEATURE_COLUMNS].values
    ...
    metadata = {
        ...
        "feature_columns": NBA_FEATURE_COLUMNS,
    }
    ...
    snapshot["feature_columns"] = NBA_FEATURE_COLUMNS
    ...
    print("\nDone. NBA production model ready.")
