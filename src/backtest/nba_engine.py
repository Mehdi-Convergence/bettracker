"""NBA backtesting engine: chronological walk-forward simulation.

Train: 2018-19 to 2022-23 (5 seasons)
Test:  2023-24 to 2024-25 (2 seasons, fixed)
...
from src.features.nba_features import NBA_FEATURE_COLUMNS, NBAFeatureBuilder
...
class NBABacktestEngine:
    """Simulate betting on NBA games historically."""
    ...
    def run(self, raw_df: pd.DataFrame) -> dict:
        """Run walk-forward backtest on raw NBA game data."""
        console.print("[bold]Building NBA features...[/bold]")
        ...
        X_train = train_df[NBA_FEATURE_COLUMNS].values.copy()
        y_train = train_df["target"].values
        X_test = test_df[NBA_FEATURE_COLUMNS].values.copy()
        ...
        console.print(f"[bold]Training NBA model...[/bold]")
        console.print(f"  Train: {len(X_train)} games ({TRAIN_SEASONS[0]}-{TRAIN_SEASONS[-1]})")
        console.print(f"  Test: {len(X_test)} games ({TEST_SEASONS[0]}-{TEST_SEASONS[-1]})")
        model = NBAModel()
    ...
    def _simulate(self, test_df: pd.DataFrame, probas: np.ndarray) -> list[dict]:
        ...
        _max_bankroll = self.initial_bankroll * 50  # sanity cap for synthetic-odds mode

        for i, row in test_df.iterrows():
            if self._check_stop_loss_total(bankroll):
                break
            if bankroll > _max_bankroll:
                bankroll = _max_bankroll  # cap to avoid float overflow with ELO synthetic odds
            ...
            # Treat NaN as missing (pandas NaN is truthy in Python, so must use pd.notna)
            ...
            # No historical odds in DB: synthesise from ELO with 5% vig.
            if not odds_home or not odds_away:
                ...
                if elo_h and elo_a and not np.isnan(float(elo_h)) and not np.isnan(float(elo_a)):
                    ...
                    # Clamp: NBA odds realistically between ~1.25 and ~3.50
                    # NBA odds rarely outside 1.55-2.40 (p 0.44-0.68 with vig)
                    p_h = max(0.42, min(0.62, p_h))
                    p_a = 1.0 - p_h
                    _vig = 1.05  # 5% total vig — realistic for NBA
            ...
            if "winner" in self.markets:
                for label, model_prob, odds, actual_won in [
                    ...
                    ("Away", prob_away, odds_away, not home_won),
                ]:
