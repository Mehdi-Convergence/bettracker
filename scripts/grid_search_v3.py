"""Grid search v3 — nouvelles features CLV + lambda pondéré + params Optuna."""
...
dr.to_csv("C:/Users/MehdiBouziane/bettracker/data/backtest_grid_v3.csv", index=False)
...
print("\n=== COMPARAISON V2 vs V3 (config ref: stake=1%, prob=55%, CLV=True) ===")
ref = dr[(dr["stake"] == 0.01) & (dr["prob"] == 0.55) & (dr["clv"] == True)]
if not ref.empty:
    r = ref.iloc[0]
    print(f"V3: ROI={r['roi']:+.2f}%, DD={r['drawdown']:.1f}%, BK={r['final_bk']:.0f}e, WR={r['wr']}%")
    print(f"V2: ROI=+8.88%, DD=4.1%, BK=267e, WR=69.2%")
