"""MLB backtesting engine: chronological walk-forward simulation.

Train: 2019-2022 (4 seasons)
Test:  2023-2025 (3 seasons, fixed)

Markets:
  - Winner (home/away moneyline)
"""
...
class MLBBacktestEngine:
    """Simulate betting on MLB games historically."""
    ...
    def __init__(
        ...
        markets: list[str] | None = None,  # ["winner"]
    ):
    ...
    def run(self, raw_df: pd.DataFrame) -> dict:
        """Run walk-forward backtest on raw MLB game data."""
        console.print("[bold]Building MLB features...[/bold]")
        ...
        X_train = train_df[MLB_FEATURE_COLUMNS].values.copy()
        y_train = train_df["target"].values
        X_test = test_df[MLB_FEATURE_COLUMNS].values.copy()
        ...
        console.print("[bold]Training MLB model...[/bold]")
    ...
    def _simulate(self, test_df: pd.DataFrame, probas: np.ndarray) -> list[dict]:
        ...
        _max_bankroll = self.initial_bankroll * 50  # sanity cap
        ...
        for i, row in test_df.iterrows():
            ...
            # No historical odds: synthesise from ELO with 5% vig
            if not odds_home or not odds_away:
                ...
                if elo_h and elo_a and not np.isnan(float(elo_h)) and not np.isnan(float(elo_a)):
                    ...
                    # MLB odds range roughly 1.35-2.80
                    p_h = max(0.38, min(0.62, p_h))
                    p_a = 1.0 - p_h
                    _vig = 1.05
