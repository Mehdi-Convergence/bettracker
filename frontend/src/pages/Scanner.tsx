  ...
  async function handleAIScan(forceRefresh = false, silent = false) {
    ...
    try {
      ...
      if (matches.length > 0 || !silent) {
        ...
        setTotalScanned(matches.length);
          ...
          <>
            <div className="flex items-center gap-3">
              ...
              >
                ...
                <span className="font-medium">Championnats</span>
          ...
          <span className="text-sm text-gray-500 flex items-center gap-2">
            ...
            <span>
              ...
              {filteredAiMatches.length !== aiMatches.length && (
                <span className="text-gray-400"> / {aiMatches.length} scannes</span>
              )}
              {filteredAiMatches.length === aiMatches.length && totalScanned > 0 && (
                <span className="text-gray-400"> | {totalScanned} scannes</span>
              )}
            </span>
          </span>
        )}
      ...
      <div className="flex gap-4 items-start">
        <div className="flex-1 min-w-0 space-y-2">
          ...
          {filteredAiMatches.map((am, i) => {
            ...
            const home = isFootball ? (am.home_team || "?") : (am.player1 || "?");
            const away = isFootball ? (am.away_team || "?") : (am.player2 || "?");
            const info = isFootball ? LEAGUE_INFO[am.league] : null;
            ...
            const qualityTip = `Fiabilite des donnees : ${Math.round((am.data_score ?? 0) * 20)}/20 `
            ...
            return (
              <div key={i} className="bg-white rounded-xl border shadow-sm transition-all group overflow-hidden border-gray-200 hover:border-gray-300 hover:shadow">
                <div className="flex items-stretch">
                  ...
                  <div
                    className="flex items-center gap-2.5 w-[26%] min-w-[210px] shrink-0 px-3 py-2.5 border-r border-gray-100 cursor-grab active:cursor-grabbing rounded-l-xl"
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className={`shrink-0 text-[9px] px-1.5 py-0.5 rounded font-bold ${
                          ...
                          {Math.round((am.data_score ?? 0) * 20)}/20
                        </span>
                        <Eye size={12} className="shrink-0 text-gray-300 group-hover:text-blue-400 transition-colors" />
                      </div>
                ...
                {/* Bottom info row — simplifié */}
                {(am.form_home || am.form_away || am.position_home != null || am.ranking_p1 != null || am.lambda_home != null || am.context || am.motivation) && (
                  <div className="px-4 py-1.5 border-t border-gray-100 bg-gray-50/60 flex flex-wrap items-center gap-3">
                    ...
                    {/* Position / classement */}
                    {isFootball && am.position_home != null && am.position_away != null && (
                      <span className="text-[10px] text-gray-500 font-medium">#{am.position_home} vs #{am.position_away}</span>
                    )}
                    {!isFootball && am.ranking_p1 != null && am.ranking_p2 != null && (
                      <span className="text-[10px] text-gray-500 font-medium">#{am.ranking_p1} vs #{am.ranking_p2}</span>
                    )}
                    {/* Lambda Poisson */}
                    {isFootball && am.lambda_home != null && (
