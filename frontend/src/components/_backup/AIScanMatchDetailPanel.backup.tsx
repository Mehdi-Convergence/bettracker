...
function Tip({ text, children }: { text: string; children: React.ReactNode }) {
  return (
    <span className="relative group cursor-help">
      {children}
      <span className="absolute bottom-full left-0 mb-1 w-56 bg-gray-900 text-white text-[10px] rounded px-2 py-1.5 hidden group-hover:block z-50 leading-relaxed shadow-lg">
        {text}
      </span>
    </span>
  );
...
export default function AIScanMatchDetailPanel({ am, home, away, onClose }: Props) {
  ...
  return (
    <>
      ...
      <div className="fixed right-0 top-0 h-full w-full max-w-2xl bg-white border-l border-gray-200 z-50 flex flex-col shadow-2xl">

        {/* Header */}
        <div className="shrink-0 px-5 py-4 border-b border-gray-200 flex items-start justify-between">
          <div>
            <h3 className="text-lg font-bold text-gray-900">{home} vs {away}</h3>
            <p className="text-sm text-gray-500 mt-0.5">
              {am.league} &bull; {dateStr}
              {am.venue && <span className="text-gray-400"> &bull; {am.venue}</span>}
              {isTennis && am.surface && <span className="text-gray-400"> &bull; {am.surface}</span>}
              {isTennis && am.round && <span className="text-gray-400"> &bull; {am.round}</span>}
            </p>
          </div>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-700 p-1 mt-1">
            <X size={20} />
          </button>
        </div>
        ...
        <div className="shrink-0 h-px bg-gray-200 mx-0" />
        ...
        <div className="flex-1 space-y-2">
          ...
          <div className="pt-3 border-t border-gray-200 space-y-1">
            <p className="text-sm text-gray-600">
              Pari recommande :{" "}
              <span className="font-bold text-gray-900">{bestOutcomeLabel}</span>
              <span className="text-gray-400 text-xs ml-1">({(bestProb * 100).toFixed(1)}% confiance)</span>
            </p>
            {bestEdge > 0 && (
  ...
  return (
    <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
      ...
      <div className="grid grid-cols-3 gap-x-2 gap-y-1 text-sm">
        ...
        <div className="text-center text-gray-400 text-[10px] self-center">5 derniers</div>
        ...
        {(am.form_home_home || am.form_away_away) && (
          <>
            ...
            <div className="text-center text-gray-400 text-[10px] self-center">Dom / Ext</div>
            <div className="flex gap-0.5">
...
function FormRow({ label, homeVal, awayVal, homeBetter }: { label: string; homeVal: string; awayVal: string; homeBetter?: boolean }) {
  return (
    <>
      <div className={`text-right text-xs py-0.5 ${homeBetter === true ? "text-emerald-600 font-semibold" : homeBetter === false ? "text-gray-400" : "text-gray-700"}`}>
        {homeVal}
      </div>
      <div className="text-center text-gray-400 text-[10px] py-0.5">{label}</div>
      <div className={`text-left text-xs py-0.5 ${homeBetter === false ? "text-emerald-600 font-semibold" : homeBetter === true ? "text-gray-400" : "text-gray-700"}`}>
        {awayVal}
      </div>
    </>
  );
  ...
  return (
    <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h4 className="text-gray-900 font-semibold text-sm">Derniers matchs</h4>
      </div>
  ...
  return (
    <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
      <div className="flex items-center gap-2 mb-3">
        ...
        {hasDetails && <span className="text-xs text-gray-400">({details.length} matchs)</span>}
      </div>
      ...
      {hasDetails && details.length > 0 && (
        <>
          ...
          {avgGoals != null && (
            <p className="text-xs text-gray-400 mb-3">{draws} nul{draws !== 1 ? "s" : ""} &bull; {avgGoals.toFixed(1)} buts/match en moy.</p>
          )}

          {/* Match list */}
          <div className="space-y-0.5">
            {details.slice(0, 8).map((m, i) => (
              <div key={i} className="flex items-center text-[11px] gap-2 py-1 border-b border-gray-100 last:border-0">
                <span className="text-gray-400 w-20 shrink-0">{m.date ? m.date.slice(0, 10) : ""}</span>
                <span className="flex-1 text-right text-gray-700 truncate">{m.home_name}</span>
                <span className="text-gray-900 font-bold w-12 text-center shrink-0">
                  {m.score_h != null ? `${m.score_h} - ${m.score_a}` : "-"}
                </span>
                <span className="flex-1 text-gray-700 truncate">{m.away_name}</span>
              </div>
            ))}
          </div>
        </>
...
function IADataTab({ am, home, away }: { am: AIScanMatch; home: string; away: string }) {
  ...
  return (
    <div className="space-y-4">
        ...
        <div>
          <div className="text-xs font-semibold text-gray-600 mb-2 flex items-center gap-1">
            ...
            <Tip text="Distribution de Poisson utilisée pour calculer les probabilités de buts attendus par équipe.">
              Notre modèle (Poisson) <span className="text-gray-400 underline decoration-dotted">ⓘ</span>
            </Tip>
          </div>
        ...
        <div>
          <div className="text-xs font-semibold text-gray-600 mb-2">
            <Tip text="Différence entre la probabilité de notre modèle et celle implicite dans la cote du bookmaker. Positif = notre modèle pense que l'issue est sous-cotée.">
              Avantage modèle vs bookmakers <span className="text-gray-400 underline decoration-dotted">ⓘ</span>
            </Tip>
          </div>
          <div className="h-28">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={edgeData} layout="vertical" margin={{ left: 80, right: 30, top: 0, bottom: 0 }}>
                <XAxis type="number" stroke="#9ca3af" tick={{ fontSize: 10 }} tickFormatter={(v) => `${v > 0 ? "+" : ""}${v}%`} />
                <YAxis type="category" dataKey="name" stroke="#9ca3af" tick={{ fontSize: 10 }} width={75} />
                <Tooltip
        ...
        <div>
          <div className="text-xs font-semibold text-gray-600 mb-2">
            <Tip text="Nombre de sources de donnees disponibles. Plus c'est eleve, plus notre modele est fiable.">
              Fiabilite des donnees <span className="text-gray-400 underline decoration-dotted">i</span>
            </Tip>
          </div>
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm text-gray-700">
              ...
              {am.data_score != null && <span className="text-gray-400 ml-1">({Math.round(am.data_score * 20)}/20 points)</span>}
            </span>
          </div>
        </div>
      )}
      ...
      {am.context && (
        <div>
          <div className="text-xs font-semibold text-gray-600 mb-1">Contexte</div>
          <p className="text-xs text-gray-600 leading-relaxed">{am.context}</p>
        </div>
      )}
      ...
      {am.motivation && (
        <div>
          <div className="text-xs font-semibold text-gray-600 mb-1">Enjeu du match</div>
          <p className="text-xs text-gray-600 leading-relaxed">{am.motivation}</p>
        </div>
      )}
      ...
      {(am.referee || am.weather) && (
        <div className="flex flex-wrap gap-4 text-[11px] text-gray-500">
          {am.referee && (
            <span>Arbitre : <span className="text-gray-700 font-medium">{am.referee}</span></span>
          )}
          {am.weather && (
            <span>Meteo : <span className="text-gray-700 font-medium">{am.weather}</span></span>
          )}
        </div>
      )}
      ...
      {((am.key_absences_home?.length ?? 0) > 0 || (am.key_absences_away?.length ?? 0) > 0) && (
        <div>
          <div className="text-xs font-semibold text-gray-600 mb-2">Absences cles</div>
          <div className="grid grid-cols-2 gap-3">
            {[{ label: home, list: am.key_absences_home }, { label: away, list: am.key_absences_away }].map(({ label, list }) => (
              <div key={label}>
                <div className="text-[10px] text-gray-500 mb-1 font-medium truncate">{label}</div>
                {list && list.length > 0 ? list.map((ab, i) => (
                  <div key={i} className="flex items-start gap-1 text-[11px] text-red-600 mb-0.5">
                    <span className="text-red-400 shrink-0 mt-0.5">•</span>
                    <span>{ab}</span>
                  </div>
                )) : <span className="text-[11px] text-gray-300">Aucune absence</span>}
              </div>
            ))}
          </div>
        </div>
...
function JoueursTab({ am, home, away }: { am: AIScanMatch; home: string; away: string }) {
  ...
  if (homePlayers.length === 0 && awayPlayers.length === 0) {
    return <p className="text-sm text-gray-400">Aucune donnee joueur disponible.</p>;
  }
    ...
    <div className="grid grid-cols-2 gap-4">
      {[{ label: home, players: homePlayers, color: "text-blue-600" }, { label: away, players: awayPlayers, color: "text-red-600" }].map(({ label, players, color }) => (
        <div key={label}>
          ...
          {players.length === 0 ? (
            <p className="text-xs text-gray-300">Aucune donnee</p>
          ) : (
            <div className="space-y-1.5">
              {players.map((p, i) => (
                <div key={i} className={`px-2 py-1.5 rounded-lg border text-[11px] ${p.is_absent ? "bg-red-50 border-red-100" : "bg-white border-gray-100"}`}>
                  <div className="flex items-center justify-between gap-1">
                    <span className={`font-medium truncate ${p.is_absent ? "text-red-700 line-through" : "text-gray-800"}`}>{p.name}</span>
                  ...
                  <div className="flex items-center gap-2 mt-0.5 text-[10px] text-gray-500">
                    ...
                    {p.goals_per_match > 0 && <span className="text-gray-400">{p.goals_per_match.toFixed(2)}/m</span>}
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-blue-50 border border-blue-100 rounded-lg p-3 text-center">
              ...
              {am.p1_age && <div className="text-[10px] text-gray-400">{am.p1_age} ans</div>}
            </div>
            <div className="bg-red-50 border border-red-100 rounded-lg p-3 text-center">
              ...
              {am.p2_age && <div className="text-[10px] text-gray-400">{am.p2_age} ans</div>}
            </div>
          </div>
        </div>
      )}
        ...
        <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
          <div className="text-xs font-semibold text-gray-600 mb-3 flex items-center gap-1">
            ...
            <Tip text={`Performance des joueurs sur ${am.surface}. Un bon bilan sur la surface est crucial en tennis.`}>
              Performance sur {am.surface} <span className="text-gray-400 underline decoration-dotted">i</span>
            </Tip>
          </div>
          <div className="grid grid-cols-2 gap-3">
            {[{ name: home, record: am.p1_surface_record, color: "blue" }, { name: away, record: am.p2_surface_record, color: "red" }].map(({ name, record, color }) => {
              ...
              return (
                <div key={name} className={`bg-${color === "blue" ? "blue" : "red"}-50 border border-${color === "blue" ? "blue" : "red"}-100 rounded-lg p-3`}>
                  <div className="text-xs font-medium text-gray-700 truncate mb-1">{name}</div>
                  <div className="text-lg font-bold text-gray-800">{record ?? "-"}</div>
        ...
        <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
          <div className="text-xs font-semibold text-gray-600 mb-3 flex items-center gap-1">
            ...
            <Tip text="% de points gagnes au service et en retour. Un bon serveur domine avec un haut % au service. Un bon retourneur excelle en retour.">
              Service & Retour <span className="text-gray-400 underline decoration-dotted">i</span>
            </Tip>
          </div>
          <div className="grid grid-cols-3 gap-x-2 gap-y-1 text-sm">
        ...
        <div>
          <div className="text-xs font-semibold text-gray-600 mb-2">
            <Tip text="Difference entre la probabilite de notre modele et celle implicite dans la cote du bookmaker.">
              Avantage modele vs bookmakers <span className="text-gray-400 underline decoration-dotted">i</span>
            </Tip>
          </div>
          <div className="h-20">
            <ResponsiveContainer width="100%" height="100%">
      ...
      {am.data_quality && (
        <div>
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm text-gray-700">
              {am.data_quality === "green" ? "Excellente" : am.data_quality === "yellow" ? "Correcte" : "Limitee"}
              {am.data_score != null && <span className="text-gray-400 ml-1">({Math.round(am.data_score * maxPts)}/{maxPts} points)</span>}
            </span>
          </div>
        </div>
      )}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
        <div className="flex items-center gap-2 mb-3">
          <TrendingUp size={15} className="text-emerald-500" />
          <h4 className="text-gray-900 font-semibold text-sm">Comparaison des joueurs</h4>
        </div>
        <div className="grid grid-cols-3 gap-x-2 gap-y-1 text-sm">
          <div className="text-right text-blue-600 font-semibold text-xs truncate">{home}</div>
          <div className="text-center text-gray-400 text-xs">VS</div>
      ...
      {(am.h2h_summary || (am.h2h_last3 && am.h2h_last3.length > 0)) && (
        <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
          ...
          {am.h2h_summary && <p className="text-xs text-gray-600 mb-2">{am.h2h_summary}</p>}
          {am.h2h_surface && (
            <p className="text-xs text-gray-500 mb-2">
              Sur {am.surface ?? "cette surface"} : <span className="font-medium text-gray-700">{am.h2h_surface}</span>
            </p>
          )}
          {am.h2h_last3 && am.h2h_last3.length > 0 && (
            <div>
              <div className="text-[10px] text-gray-500 mb-1 font-medium">3 derniers matchs :</div>
              <div className="space-y-0.5">
                {am.h2h_last3.map((m, i) => (
                  <div key={i} className="text-[11px] text-gray-600 py-0.5">{m}</div>
                ))}
              </div>
            </div>
          )}
      ...
      {((am.key_absences_home?.length ?? 0) > 0 || (am.key_absences_away?.length ?? 0) > 0) && (
        <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
          <div className="text-xs font-semibold text-gray-600 mb-2">Blessures / Absences</div>
          <div className="grid grid-cols-2 gap-3">
            {[{ label: home, list: am.key_absences_home }, { label: away, list: am.key_absences_away }].map(({ label, list }) => (
              <div key={label}>
                <div className="text-[10px] text-gray-500 mb-1 font-medium truncate">{label}</div>
...
function CotesTab({ am }: { am: AIScanMatch }) {
  ...
  if (!odds || Object.keys(odds).length === 0) {
    return <p className="text-sm text-gray-400">Aucune cote disponible.</p>;
  }
            ...
            {isCard ? (
              ...
              <div className={`grid gap-2 ${entries.length === 2 ? "grid-cols-2" : "grid-cols-3"}`}>
                {entries.map(([outcome, val]) => {
                  ...
                  return (
                    <div key={outcome} className={`flex flex-col rounded-xl overflow-hidden border-2 bg-white shadow-sm ${isFav ? "border-emerald-300" : "border-gray-100"}`}>
                      {/* Prob bar top */}
                      {market === mainMarket && prob > 0 && (
                        <div className={`h-1 w-full ${isFav ? "bg-emerald-400" : "bg-gray-200"}`} />
                      )}
                      {/* Card body */}
                      <div className="flex flex-col items-center px-2 pt-2.5 pb-2 gap-1">
                        <span className="text-[10px] font-medium text-gray-500 text-center leading-tight w-full truncate text-center uppercase tracking-wide">
                          {outcome}
                        </span>
                        <span className={`text-2xl font-bold tabular-nums ${isFav ? "text-emerald-600" : "text-gray-800"}`}>
                          {currentOdd.toFixed(2)}
                        </span>
                        {market === mainMarket && prob > 0 && (
                          <span className={`text-[9px] font-medium ${isFav ? "text-emerald-500" : "text-gray-400"}`}>
                            {(prob * 100).toFixed(0)}%
                          </span>
                        )}
                        {bkOptions.length > 1 ? (
                          <select
                            className="text-[10px] text-gray-400 bg-gray-50 border border-gray-100 rounded px-1 py-0.5 cursor-pointer focus:outline-none w-full text-center mt-0.5"
                            value={currentBk}
                          ...
                          <span className="text-[10px] text-gray-400 mt-0.5">{bestBk.bk}</span>
                        )}
                      </div>
                    </div>
                  );
              ...
              <div className="divide-y divide-gray-50 border border-gray-100 rounded-xl overflow-hidden">
                {entries.map(([outcome, val]) => {
                  ...
                  return (
                    <div key={outcome} className="flex items-center gap-3 bg-white px-3 py-2.5 hover:bg-gray-50 transition-colors">
                      <span className="flex-1 text-sm text-gray-600 truncate">{outcome}</span>
                      <span className="text-base font-bold tabular-nums text-gray-900 min-w-[3rem] text-right">
                        {currentOdd.toFixed(2)}
                      </span>
                      {bkOptions.length > 1 ? (
                        <select
                          className="text-[11px] text-gray-500 bg-gray-50 border border-gray-100 rounded px-1.5 py-0.5 cursor-pointer focus:outline-none focus:border-blue-300 max-w-[110px]"
                          value={currentBk}
                          onChange={(e) => setSelectedBk((prev) => ({ ...prev, [key]: e.target.value }))}
                        >
                          {bkOptions.map(({ bk, odd }) => (
                        ...
                        <span className="text-[11px] text-gray-400 max-w-[110px] truncate">{bestBk.bk}</span>
                      )}
                    </div>
                  );
                })}
...
function IAConseilTab({ am, home, away }: { am: AIScanMatch; home: string; away: string }) {
  ...
  if (sentences.length === 0) {
    return <p className="text-sm text-gray-400">Donnees insuffisantes pour generer une analyse.</p>;
  }
    ...
    <div>
      <div className="flex items-center gap-2 mb-3">
        <span className={`text-[10px] px-2 py-0.5 rounded font-medium ${
          ...
          am.lineup_status === "presumed" ? "bg-gray-100 text-gray-500" :
      ...
      {lineupH.length === 0 && lineupA.length === 0 ? (
        <p className="text-sm text-gray-400">Composition non disponible pour ce match.</p>
      ) : (
        <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-0.5">
                {lineup.map((p, i) => (
                  <div key={i} className="flex items-center gap-2 text-[11px] text-gray-600">
                    {p.number != null && <span className="text-gray-300 w-4 text-right text-[9px] shrink-0">{p.number}</span>}
                    <span className="text-[9px] text-gray-400 w-5 shrink-0">{p.pos}</span>
                    <span>{p.name}</span>
                  </div>
                ))}
              </div>
