...
dr.to_csv("C:/Users/MehdiBouziane/bettracker/data/backtest_grid_v4.csv", index=False)
...
print("\n=== COMPARAISON V3 vs V4 (config ref: stake=1%, prob=55%, CLV=True) ===")
ref = dr[(dr["stake"] == 0.01) & (dr["prob"] == 0.55) & (dr["clv"] == True)]
if not ref.empty:
    r = ref.iloc[0]
    print(f"V4: ROI={r['roi']:+.2f}%, DD={r['drawdown']:.1f}%, BK={r['final_bk']:.0f}e, WR={r['wr']}%, Bets={r['bets']}")
    print(f"V3: ROI=+9.21%, DD=6.2%, BK=482e, WR=68.3%, Bets=1301")
