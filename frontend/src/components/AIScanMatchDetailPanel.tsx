...
const TABS: { key: Tab; label: string }[] = [
  ...
  { key: "equipes", label: "Equipes & Stats" },
...
function PredictionSection({ am, home, away }: { am: AIScanMatch; home: string; away: string }) {
  ...
  const bestOutcome = probH >= probD && probH >= probA ? "H" : probD >= probH && probD >= probA ? "D" : "A";
  const bestOutcomeLabel = bestOutcome === "H" ? home : bestOutcome === "D" ? "Nul" : away;
  const bestProb = bestOutcome === "H" ? probH : bestOutcome === "D" ? probD : probA;
      ...
      <div className="flex items-center gap-2 mb-3">
        ...
        {am.data_quality && (
          <span className={`ml-auto text-[10px] px-2 py-0.5 rounded-full font-medium ${
            ...
            {am.data_score != null ? `Donnees ${Math.round(am.data_score * 20)}/20` : am.data_quality}
          </span>
        )}
      </div>
          ...
          <div className="pt-3 border-t border-gray-200 space-y-1">
            ...
            {bestEdge > 0 && (
              <p className="text-sm text-gray-500">
                Value bet :{" "}
                <span className={`font-semibold ${bestEdgeKey === "H" ? "text-blue-600" : bestEdgeKey === "D" ? "text-amber-600" : "text-red-600"}`}>
  ...
  return (
    <div className="space-y-4">
      ...
      {am.data_quality && (
        <div>
          <div className="text-xs font-semibold text-gray-600 mb-2">
            <Tip text="Nombre de sources de données disponibles sur 20. Plus c'est élevé, plus notre modèle est fiable.">
              Fiabilité des données <span className="text-gray-400 underline decoration-dotted">ⓘ</span>
            </Tip>
          </div>
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm text-gray-700">
              {am.data_quality === "green" ? "Excellente" : am.data_quality === "yellow" ? "Correcte" : "Limitée"}
      ...
      {(am.referee || am.weather) && (
        <div className="flex flex-wrap gap-4 text-[11px] text-gray-500">
          ...
          {am.weather && (
            <span>☁ Météo : <span className="text-gray-700 font-medium">{am.weather}</span></span>
          )}
        </div>
      )}
...
const CARD_MARKETS = new Set(["1x2", "btts", "double_chance", "draw_no_bet"]);
...
function CotesTab({ am }: { am: AIScanMatch }) {
  ...
  const MARKET_ORDER = ["1x2", "btts", "over_under_2.5", "over_under", "double_chance", "draw_no_bet", "asian_handicap", "goalscorer_anytime", "goalscorer_first"];
  const MARKET_LABELS: Record<string, string> = {
    ...
    "goalscorer_anytime": "Buteur (à tout moment)",
  ...
  // Model probs for 1X2 probability bar
  const modelProbs: Record<string, number> = {
    H: am.model_prob_home ?? 0,
    D: am.model_prob_draw ?? 0,
    A: am.model_prob_away ?? 0,
  };
            ...
            {isCard ? (
              ...
              <div className={`grid gap-2 ${entries.length === 2 ? "grid-cols-2" : "grid-cols-3"}`}>
                {entries.map(([outcome, val]) => {
                  ...
                  const prob = market === "1x2" ? (modelProbs[outcome] ?? 0) : 0;
                  const isFav = market === "1x2" && prob === maxProb && maxProb > 0;
...
function IAConseilTab({ am, home, away }: { am: AIScanMatch; home: string; away: string }) {
  ...
  const bestOutcome = probH >= probD && probH >= probA ? "H" : probD >= probA ? "D" : "A";
  const bestLabel = bestOutcome === "H" ? home : bestOutcome === "D" ? "Nul" : away;
  const bestProb = bestOutcome === "H" ? probH : bestOutcome === "D" ? probD : probA;
