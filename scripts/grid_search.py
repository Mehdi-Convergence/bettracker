...
dr.to_csv("C:/Users/MehdiBouziane/bettracker/data/backtest_grid_v6.csv", index=False)
...
print("\n=== COMPARAISON V5 vs V6 (config ref: stake=1%, prob=55%, CLV=True) ===")
ref = dr[(dr["stake"] == 0.01) & (dr["prob"] == 0.55) & (dr["clv"] == True)]
if not ref.empty:
    r = ref.iloc[0]
    print(f"V6: ROI={r['roi']:+.2f}%, DD={r['drawdown']:.1f}%, BK={r['final_bk']:.0f}e, WR={r['wr']}%, Bets={r['bets']}")
    print(f"V5 (ensemble 80/20, depth=5): ROI=+8.23%, DD=6.4%, BK=495e, WR=66.1%, Bets=1500")
