...
export default function Parametres() {
  const { user } = useAuth();
  const { updatePreferences: syncContext } = usePreferences();
  ...
  const handleSave = async (section: SectionId) => {
    ...
    try {
      ...
      setPrefs(updated);
      // Synchroniser le context global (applique le theme, la langue, etc.)
      if (dirty.theme !== undefined || dirty.language !== undefined || dirty.currency !== undefined) {
        await syncContext(dirty);
      }
      setDirty({});
      setSaved(section);
    ...
    <div className="animate-fade-up overflow-x-hidden">
      ...
      <div className="flex flex-col md:flex-row gap-5">
        ...
        <div className="flex-1 min-w-0 flex flex-col gap-5">
          ...
          {/* 2FA deplace dans Profil > Securite */}
        </div>
      </div>
